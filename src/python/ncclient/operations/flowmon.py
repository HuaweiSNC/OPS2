...
HUAWEI_1_0="http://www.huawei.com/netconf/capability/base/1.0"
...
class ExecuteAction(RPC):                                 #maohongsen
    "*execute* a action"
   
    def request(self,hwcontext,actionreq=None):
        
        self.__setattr__('hwcontext',hwcontext)
        execute_action=new_ele(qualify("execute-action",None),xmlns=HUAWEI_1_0)
        action=sub_ele(execute_action,"action",xmlns=HUAWEI_1_0)
        fNode=to_ele(actionreq)
        action.append(fNode)
        return self._request(execute_action)
