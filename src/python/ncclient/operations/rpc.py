...
class RPCReplyListener(SessionListener): # internal use
    ...
    def callback(self, root, raw):
        tag, attrs = root
        if raw.find('<active')!=-1:
            for key in attrs:
                if key == "message-id":
                    logger.debug('delivering the active response to increase the session timeout =%s', raw)
                    id = attrs[key]
                    rpc = self._id2rpc[id]
                    rpc.deliver_reply(raw)
        
        if (tag!="rpc-reply") and tag != qualify("rpc-reply"):
            return
...
class RPC(object):
    ...
    def _wrap(self, subele):
        # internal use
        hwcontext = self.__getattribute__('hwcontext')
        if '' != hwcontext:
            ele = new_ele("rpc", {"message-id": self._id,"hwcontext":hwcontext})
        else:
...
#                 logger.debug('Sync request, will wait for timeout=%r' % self._timeout)
                ...
                sreply = str(self._reply)
                if sreply.find('<active')!=-1:
                   #print"[RPC]rply",sreply
                    self._reply = None ## we making the reply empty as the time out is extended
                    logger.debug(' due to active info exchange, session time is increased by =%r' % self._timeout)
                    logger.debug('client is waiting for the response')
#                    self._event.wait(self._timeout)
                    continue## will increase the session timeout
                elif not self._event.isSet():
                    #print "[rpc]reply need to be expect"
                    logger.debug(' due to reply expect delimiter, session time is increased by =%r' % self._timeout)
                    logger.debug('client is waiting for the complete response')
                    continue## will increase the session timeout
                elif self._event.isSet():## need to take care if the reply is none...
                    #print
                    if self._error:
                        raise self._error
