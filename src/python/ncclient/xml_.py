...
HUAWEI_VRP = "http://www.huawei.com/netconf/vrp"
#
try:
...
def to_xml(ele, encoding="UTF-8"):
    ...
    xml = ET.tostring(ele) #maohongsen , encoding
...
new_ele = lambda tag, attrs=None, **extra: ET.Element(qualify(tag), attrs, **extra)

sub_ele = lambda parent, tag, attrs=None, **extra: ET.SubElement(parent, qualify(tag), attrs, **extra)

"""
def new_ele(tag,attrs=None,**extra):
    return ET.Element(qualify(tag), attrs, **extra)

def sub_ele(parent,tag,attrs=None,**extra):
    return ET.SubElement(parent, qualify(tag), attrs, **extra)
"""
if __name__ == "__main__":
    filter="""<aaa xmlns="mao"><lam xmlns="hongsen">10</lam></aaa>"""
    node=new_ele("rpc",xmlns="huawei")
    doc=to_ele(filter)
    sub=sub_ele(node,"get",xmlns="com")
    node.append(sub)
    sub.append(doc)
    print to_xml(node)
